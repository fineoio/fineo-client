...
import io.fineo.read.auth.cognito.exception.CognitoInternalErrorException;
import io.fineo.read.auth.cognito.exception.CognitoNotAuthorizedException;
...
import io.fineo.read.auth.cognito.handle.GenericHandler;
import io.fineo.read.auth.cognito.token.CognitoAccessToken;
import io.fineo.read.auth.cognito.token.CognitoDeviceHelper;
import io.fineo.read.auth.cognito.token.CognitoIdToken;
import io.fineo.read.auth.cognito.token.CognitoRefreshToken;
    ...
    private CognitoUserSession readCachedTokens() {
        ...
        try {
            ...
            if (tokenCache.containsKey(csiIdTokenKey)) {
                CognitoIdToken csiCachedIdToken = new CognitoIdToken(tokenCache.getOrDefault(csiIdTokenKey, null));
