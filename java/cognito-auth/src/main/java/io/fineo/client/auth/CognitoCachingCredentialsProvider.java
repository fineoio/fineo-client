 ...
 * ability to persist the Cognito identity id in an internal cache.
...
public class CognitoCachingCredentialsProvider
    ...
    private static final Logger LOG = LoggerFactory.getLogger(CognitoCachingCredentialsProvider.class);

    private final String DEFAULT_SHAREDPREFERENCES_NAME = "com.amazonaws.android.io.fineo.client.auth.auth";
    ...
    private final IdentityChangedListener listener = new IdentityChangedListener() {
        ...
        public void identityChanged(String oldIdentityId, String newIdentityId) {
            LOG.debug(TAG, "Identity id is changed");
     ...
     * After that it's saved in the cache. Please don't call it
     * in the main thread.
     *
    ...
    synchronized public AWSSessionCredentials getCredentials() {
        ...
        try {
            ...
            LOG.error(TAG, "Failure to get credentials", e);
            if (getLogins() != null) {
    ...
    /*
     * (non-Javadoc)
     * @see com.amazonaws.io.fineo.client.auth.auth.CognitoCredentialsProvider#clear() Clears the AWS
    ...
    /*
     * (non-Javadoc)
     * @see com.amazonaws.io.fineo.client.auth.auth.CognitoCredentialsProvider#clearCredentials()
     * Clears the AWS credentials
     */
    @Override
    public void clearCredentials() {
        super.clearCredentials();
        LOG.debug(TAG, "Clearing credentials from SharedPreferences");
        for(String key: new String[]{AK_KEY, SK_KEY, ST_KEY, EXP_KEY}){
            cache.remove(key);
        }
    }
    ...
    public String getCachedIdentityId() {
        String cachedIdentityId = cache.getOrDefault(namespace(ID_KEY), null);
    ...
    void loadCachedCredentials() {
        ...
        sessionCredentialsExpiration =
          new Date(Long.parseLong(cache.getOrDefault(namespace(EXP_KEY), "0")));
    ...
    private void saveIdentityId(String identityId) {
        LOG.debug(TAG, "Saving identity id to SharedPreferences");
        this.identityId = identityId;
        cache.put(namespace(ID_KEY), identityId);
    }
